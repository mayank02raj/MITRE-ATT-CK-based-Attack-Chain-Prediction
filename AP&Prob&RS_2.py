# AP_Prob_RS_2.py  (stream-safe, dataset-anchored predictions)
# End-to-end pipeline in your requested order:
...
UNIT42_REPO_DIR = os.environ.get("UNIT42_REPO_DIR",
    "/Users/mayankraj/repos/playbook_viewer")  # must contain playbook_json/
ATTACK_FLOW_STIX_DIR = os.environ.get("ATTACK_FLOW_STIX_DIR",
    "/Users/mayankraj/data/attack_flow_examples")  # folder of *.json flows
...
# =========================================================
# 1) Load MITRE ATT&CK Excel + maps
# =========================================================
...
for _, crow in tqdm(camp_df.iterrows(), total=len(camp_df), desc="Campaigns", leave=False):
    ...
    if cs:
        ...
        pre_camp_names.extend([crow.get("name","<unknown>")] * len(cs))
...
for cid, (camp_name, chain) in enumerate(zip(pre_camp_names, pre_chains), start=1):
...
def score_chain(seq_names, octave_impact_0_10=OCTAVE_IMPACT_0_10_DEFAULT):
    ...
    # Clamp geo-mean to [MARKOV_MIN_PROB, 1] just in case
    ...
    final_risk = min(10.0, final_risk)  # ---- Risk scale fix: cap at 10
    return {
...
# Save POST-LSTM
post_rows = []
for cid, (camp_name, chain) in enumerate(zip(campaign_index, all_chains), start=1):
...
if not post_mk_sorted.empty:
    ...
    plt.title('Distribution of Risk Scores Across 26,051 Predictions', fontsize=14)
    plt.axvline(x=7.0, color='orange', linestyle='--', linewidth=2, label='73% threshold (Risk>7.0)')
...
if os.path.exists(val_csv):
    ...
    plt.ylabel('Frequency (Number of Chains)', fontsize=12)
    plt.title('LSTM-Markov Coverage Distribution (4,849 Chains)', fontsize=14)
